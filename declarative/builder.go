...
var (
	conditionsByName = make(map[string]winapi.Condition)
	propertyRE       *regexp.Regexp
)

func init() {
	winapi.AppendToWalkInit(func() {
		propertyRE = regexp.MustCompile("[A-Za-z]+[0-9A-Za-z]*(\\.[A-Za-z]+[0-9A-Za-z]*)+")
	})
}

func MustRegisterCondition(name string, condition winapi.Condition) {
	if name == "" {
		panic(`name == ""`)
	}
	if condition == nil {
...
type Builder struct {
	...
	parent                   winapi.Container
	declWidgets              []declWidget
	name2Window              map[string]winapi.Window
	name2DataBinder          map[string]*winapi.DataBinder
	deferredFuncs            []func() error
	knownCompositeConditions map[string]winapi.Condition
	expressions              map[string]winapi.Expression
	functions                map[string]govaluate.ExpressionFunction
}

func NewBuilder(parent winapi.Container) *Builder {
	var dpi int
	...
	return &Builder{
		dpi:                      dpi,
		parent:                   parent,
		name2Window:              make(map[string]winapi.Window),
		name2DataBinder:          make(map[string]*winapi.DataBinder),
		knownCompositeConditions: make(map[string]winapi.Condition),
		expressions:              make(map[string]winapi.Expression),
		functions:                make(map[string]govaluate.ExpressionFunction),
	}
}

func (b *Builder) Parent() winapi.Container {
	return b.parent
}
...
func (b *Builder) deferBuildMenuActions(menu *winapi.Menu, items []MenuItem) {
	if len(items) > 0 {
		b.Defer(func() error {
...
func (b *Builder) deferBuildActions(actionList *winapi.ActionList, items []MenuItem) {
	if len(items) > 0 {
		b.Defer(func() error {
			for _, item := range items {
...
func (b *Builder) InitWidget(d Widget, w winapi.Window, customInit func() error) error {
	if b.dpi == 0 {
		b.dpi = w.DPI()
	}
	...
	if widget, ok := w.(winapi.Widget); ok {
		if alignment := b.alignment(); alignment != AlignHVDefault {
			if err := widget.SetAlignment(winapi.Alignment2D(alignment)); err != nil {
				return err
			}
		}
		...
		if field := b.widgetValue.FieldByName("GraphicsEffects"); field.IsValid() {
			for _, effect := range field.Interface().([]winapi.WidgetGraphicsEffect) {
		...
		if p := widget.Parent(); p != nil {
			type SetStretchFactorer interface {
				SetStretchFactor(widget winapi.Widget, factor int) error
			}
			...
			case *winapi.GridLayout:
				...
				r := winapi.Rectangle{column, row, columnSpan, rowSpan}
	...
	// Container
	var db *winapi.DataBinder
	if wc, ok := w.(winapi.Container); ok {
		var layout Layout
		...
		type DelegateContainerer interface {
			DelegateContainer() winapi.Container
		}
		...
		if dataBinder.AssignTo != nil || dataBinder.DataSource != nil {
			if dataB, err := dataBinder.create(); err != nil {
				...
				if ep := db.ErrorPresenter(); ep != nil {
					if dep, ok := ep.(winapi.Disposable); ok {
...
func (b *Builder) initAccessibility(d Widget, w winapi.Window) error {
	...
	if accessibility.IsValid() {
		...
		if a.Role > 0 {
			w.Accessibility().SetRole(winapi.AccRole(a.Role))
		}
		...
		if a.State > 0 {
			w.Accessibility().SetState(winapi.AccState(a.State))
		}
...
func (b *Builder) eventHandler(fieldName string) winapi.EventHandler {
	fieldValue := b.widgetValue.FieldByName(fieldName)

	if fieldValue.IsValid() {
		return fieldValue.Interface().(winapi.EventHandler)
	}
...
func (b *Builder) keyEventHandler(fieldName string) winapi.KeyEventHandler {
	fieldValue := b.widgetValue.FieldByName(fieldName)

	if fieldValue.IsValid() {
		return fieldValue.Interface().(winapi.KeyEventHandler)
	}
...
func (b *Builder) mouseEventHandler(fieldName string) winapi.MouseEventHandler {
	fieldValue := b.widgetValue.FieldByName(fieldName)

	if fieldValue.IsValid() {
		return fieldValue.Interface().(winapi.MouseEventHandler)
	}
...
func (b *Builder) conditionOrProperty(data Property) interface{} {
	...
	case bindData:
		...
		e := &expression{
			text:           val.expression,
			subExprsByPath: subExpressions(make(map[string]winapi.Expression)),
		}

		var singleExpr winapi.Expression
				...
				if w, ok := b.name2Window[parts[0]]; ok {
					if prop := w.AsWindowBase().Property(parts[1]); prop != nil {
						...
						if len(parts) == 2 {
							...
							e.addSubExpression(s, winapi.NewReflectExpression(prop, s[len(parts[0])+len(parts[1])+2:]))
						}
					} else {
					...
					e.addSubExpression(s, winapi.NewReflectExpression(expr, s[len(parts[0])+1:]))
				}
			}
...
type expression struct {
	...
	changedPublisher       winapi.EventPublisher
	lastReportedValue      interface{}
}

type subExpressions map[string]winapi.Expression
...
func (e *expression) Changed() *winapi.Event {
	return e.changedPublisher.Event()
}

func (e *expression) addSubExpression(path string, subExpr winapi.Expression) {
