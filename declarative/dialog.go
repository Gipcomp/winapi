...
type Dialog struct {
	...
	AssignTo      **winapi.Dialog
	CancelButton  **winapi.PushButton
	DefaultButton **winapi.PushButton
	FixedSize     bool
}

func (d Dialog) Create(owner winapi.Form) error {
	var w *winapi.Dialog
	var err error

	if d.FixedSize {
		w, err = winapi.NewDialogWithFixedSize(owner)
	} else {
		w, err = winapi.NewDialog(owner)
	}
	if err != nil {
		return err
	}
...
func (d Dialog) Run(owner winapi.Form) (int, error) {
	var w *winapi.Dialog
